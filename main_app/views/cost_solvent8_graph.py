...
class CostSolvent8GraphView(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self,**kwargs):
        ...
        #################################################
        df = read_frame(queryset,fieldnames=['Data_date','Cost_solvent8','Machine_model'])
        
        gen = GraphGenerator()

        # pieチャートの素材を作成
        df_pie = pd.pivot_table(df,index='Machine_model',values='Cost_solvent8',aggfunc=np.sum)
