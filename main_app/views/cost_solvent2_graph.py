...
class CostSolvent2GraphView(LoginRequiredMixin,TemplateView):
    template_name = 'cost_graph/cost_solvent2_graph.html'

    def get_context_data(self,**kwargs):
        ctx = super().get_context_data(**kwargs)
        
        ctx['title'] = '溶剤2コスト詳細'
        ctx['msg'] = '溶剤2コスト詳細確認が出来ます。'
        ...
        ctx['table_set'] = df_pie.to_dict()['Cost_solvent2']

        # totalの数字を計算して渡す
        ctx['total_payment'] = df['Cost_solvent2'].sum()

        # 日別の棒グラフの素材を渡す
        df_bar = pd.pivot_table(df, index='Data_date', values='Cost_solvent2', aggfunc=np.sum)
