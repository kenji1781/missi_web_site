...
class SolventManufacturerView(ListView):
    ...
    def get_queryset(self):
        ...
        if q_word:
            object_list = Solvent_Manufacturer.objects.filter(Q(Solvent_name__icontains=q_word))
        elif q_date:
            object_list = Solvent_Name.objects.filter(Q(Solvent_manu_input_date__icontains=q_date))
        else:
