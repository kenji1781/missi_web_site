...
class CostGasGraphView(LoginRequiredMixin,TemplateView):
    template_name = 'cost_graph/cost_gas_graph.html'

    def get_context_data(self,**kwargs):
        ctx = super().get_context_data(**kwargs)
        
        ctx['title'] = 'ガスコスト詳細'
        ctx['msg'] = 'ガスコスト詳細確認が出来ます。'
        ...
        #################################################
        df = read_frame(queryset,fieldnames=['Data_date','Cost_gas','Customer_machine_recipe'])
        
        gen = GraphGenerator()

        # pieチャートの素材を作成
        df_pie = pd.pivot_table(df,index='Customer_machine_recipe',values='Cost_gas',aggfunc=np.sum)
