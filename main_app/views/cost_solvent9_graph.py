...
class CostSolvent9GraphView(LoginRequiredMixin,TemplateView):
    template_name = 'cost_graph/cost_solvent9_graph.html'

    def get_context_data(self,**kwargs):
        ctx = super().get_context_data(**kwargs)
        
        ctx['title'] = '溶剤9コスト詳細'
        ctx['msg'] = '溶剤9コスト詳細確認が出来ます。'
        ...
        #################################################
        df = read_frame(queryset,fieldnames=['Data_date','Cost_solvent9','Customer_machine_recipe'])
        
        gen = GraphGenerator()

        # pieチャートの素材を作成
        df_pie = pd.pivot_table(df,index='Customer_machine_recipe',values='Cost_solvent9',aggfunc=np.sum)
        ...
        ctx['table_set'] = df_pie.to_dict()['Cost_solvent9']

        # totalの数字を計算して渡す
        ctx['total_payment'] = df['Cost_solvent9'].sum()
